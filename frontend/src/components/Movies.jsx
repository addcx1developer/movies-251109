      ...
      <table className="min-w-full text-left border-collapse">
        <thead>
          <tr className="border-b bg-gray-100">
            ...
            <th className="px-2 py-2 font-semibold text-gray-700">Release Date</th>
            <th className="px-2 py-2 font-semibold text-gray-700">Rating</th>
          </tr>
        </thead>
        <tbody>
          {movies.map((m) => (
            <tr key={m.id} className="odd:bg-white even:bg-gray-50 hover:bg-gray-100">
              <td className="px-2 py-2">
                <Link to={`/movies/${m.id}`} className="text-blue-600 hover:text-blue-800 font-medium">
                  {m.title}
                </Link>
              </td>
